...
const userId =localStorage.getItem('userId');
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        const response = await fetch('http://localhost:8000/api/checkup/requests', {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
  ...
  const handleUpload = async (checkupRequestId) => {
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        const response = await fetch('http://localhost:8000/api/checkup/photo', {
          method: 'POST',
          headers: {
        ...
        if (response.ok) {
          ...
          const updatedResponse = await fetch('http://localhost:8000/api/checkup/requests', {
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('token')}`
            }
          });
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {requests.map((request) => (
                    <tr key={request._id}>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{request.userId.email}</td>
